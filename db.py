...
def start_new_league(name: str, player_ids: list[str]):
    ...
    home & away round-robin fixture list."""
...
def list_fixtures(league_id: str):
    ...
    return sb.table("fixtures").select(
        "*, home:home_player_id(id, club_name, ign), away:away_player_id(id, club_name, ign)"
...
def get_standings(league_id: str):
    """Computes the league table from played fixtures. Returns a list of
    dicts sorted by points, goal diff, goals for."""
    fixtures = list_fixtures(league_id)
    ...
    for f in fixtures:
        for side in ("home", "away"):
            ...
            if pid not in stats and pid in players:
                p = players[pid]
                stats[pid] = {
                    "player_id": pid, "club_name": p["club_name"], "ign": p["ign"],
        ...
        h, a = stats.get(f["home_player_id"]), stats.get(f["away_player_id"])
        if not h or not a:
            continue
        h["played"] += 1
        a["played"] += 1
